...
const exportExercises = async () => {
    ...
    const forceArray= Array.from(forceList).map((force,index) => ({id: index.toString(), name: force}))
    const levelArray = Array.from(levelList).map((level,index) => ({id: index.toString(), name: level}))
    const mechanicArray = Array.from(mechanicList).map((mechanic,index) => ({id: index.toString(), name: mechanic}))
    const equipmentArray = Array.from(equipmentList).map((equipment,index) => ({id: index.toString(), name: equipment}))
    const muscleArray = Array.from(muscleList).map((muscle,index) => ({id: index.toString(), name: muscle}))
    const categoryArray = Array.from(categoryList).map((category,index) => ({id: index.toString(), name: category}))
    ...
    const exercises = exercisesRow.map((exercise) => {
        const newExercise= {
            ...
            mechanic: mechanicArray.find((mechanic) => mechanic.name === exercise.mechanic) ,
            equipment: equipmentArray.find((equipment) => equipment.name === exercise.equipment) ,
            instructions: exercise.instructions,
            category: categoryArray.find((category) => category.name === exercise.category) ,
            primaryMuscles: exercise.primaryMuscles.map((muscle) => muscleArray.find((muscleItem) => muscleItem.name === muscle) ),
        secondaryMuscles: exercise.secondaryMuscles.map((muscle) => muscleArray.find((muscleItem) => muscleItem.name === muscle) ),
    }
        return newExercise
    })
