...
export const saveSession = createAsyncThunk(workoutSessionActionNames.saveSession, async (args, thunkAPI) => {
    try {
        ...
        if (state.workoutSession.workoutSession && user) {
            ...
            // Refresh the list
            await thunkAPI.dispatch(workoutSessionInfiniteListActions.refreshData())
            return
        }
...
export const editSession = createAsyncThunk(workoutSessionActionNames.editSession, async (args, thunkAPI) => {
    try {
        const state = thunkAPI.getState() as RootState;
        const user = UserService.getAuthUser()
        if (state.workoutSession.workoutSession && user) {
            ...
            await UserWorkoutService.saveUserWorkoutSession(session.id)
            // Save the session Hit in Typesense
            await workoutSessionTypesenseService.updateWorkoutSession(session)
            // Refresh the list
            await thunkAPI.dispatch(workoutSessionInfiniteListActions.refreshData())
            // Update the session detail
            thunkAPI.dispatch(workoutSliceActions.setWorkoutProps({workoutSessionDetail: session}))
            return
        }
        throw new Error('workoutSession not found')
    } catch (e: any) {
