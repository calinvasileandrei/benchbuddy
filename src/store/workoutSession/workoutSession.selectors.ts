...
const getCurrentExerciseSession = (state: RootState, exerciseId: string): ExerciseWorkoutModel | undefined => {
    if (state.workoutSession.workoutSession === undefined || state.workoutSession.workoutSession.sessionExercises.length == 0) return;
    return state.workoutSession.workoutSession?.sessionExercises.find((exercise) => exercise.id === exerciseId)
}
