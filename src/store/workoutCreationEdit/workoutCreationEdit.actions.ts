...
const saveWorkout = createAsyncThunk(workoutCreationActionNames.saveWorkout, async (args, thunkAPI) => {
    try {
        const state = thunkAPI.getState() as RootState;
        const workout = state.workoutCreationEdit.workout;
        if (workout) {
            await UserWorkoutService.saveUserWorkout(workout)
            await thunkAPI.dispatch(workoutSliceActions.saveWorkout(workout))
        }
    } catch (e: any) {
        throw new Error('saveWorkout error', e.message)
    }
});

const editWorkout = createAsyncThunk(workoutCreationActionNames.editWorkout, async (args, thunkAPI) => {
    try {
        const state = thunkAPI.getState() as RootState;
        ...
        if (workout && workoutEdited) {
            // save on firestore
            await UserWorkoutService.editUserWorkout(workout,workoutEdited)
            // update the detail workout
            thunkAPI.dispatch(workoutSliceActions.setWorkoutProps({detailWorkout: workoutEdited}))
            // update the workout list
            thunkAPI.dispatch(workoutSliceActions.editWorkout(workoutEdited))
        }
    } catch (e: any) {
        throw new Error('editWorkout error', e.message)
    }
});
