...
export const workoutCreationEditSlice = createSlice({
    ...
    reducers: {
        ...
        saveExerciseSet: (state, action: PayloadAction<{sessionExerciseId: string, exerciseSets: ExerciseSetModel[]}>) => {
            const {sessionExerciseId, exerciseSets} = action.payload;
            if (state.workout){
                const index = state.workout?.exercises.findIndex((exerciseWorkout) => exerciseWorkout.id === sessionExerciseId)
                if (index === -1) {
                    logger.debug('Exercise not found in the list, nothing to update')
                    return
                }
                const exerciseWorkout = [...state.workout?.exercises];
                exerciseWorkout[index] = {...state.workout.exercises[index],exerciseSets}
        ...
        deleteExercise: (state, action: PayloadAction<string>) => {
            ...
            if (state.workout?.exercises){
                const index = state.workout?.exercises.findIndex((exerciseWorkout) => exerciseWorkout.exercise.id === exerciseId)
                const exerciseWorkout = state.workout?.exercises;
