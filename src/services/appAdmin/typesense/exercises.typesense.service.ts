...
const initAndPopulate = async () => {
    ...
    const exercisesForTypesense = exercisesArray.map(
    ...
    console.log(
        'Loaded data, ready to populate typesense length:',
        exercisesForTypesense.length,
    );
    ...
    try {
        ...
        if (collection.num_documents > 0) {
            ...
            console.log(
                'Collection exists but has no documents or delete failed',
            );
        }
    } catch (e) {
