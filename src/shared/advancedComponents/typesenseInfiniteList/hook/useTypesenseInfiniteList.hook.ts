...
export const useTypesenseInfiniteList = <T>({
    ...
    useEffect(() => {
        if (pageSize) {
            dispatch(typesenseInfiniteListSliceActions.setDefaultPageSize(pageSize))
        }
    ...
    useEffect(() => {
        dispatch(typesenseInfiniteListSliceActions.setSearchParameter(searchTextParam))
    ...
    const fetchMoreData = async (params: FetchMoreDataParams | undefined) => {
        ...
        await dispatch(typesenseInfiniteListActions.fetchData<T>()({
            col,
            orderValue,
