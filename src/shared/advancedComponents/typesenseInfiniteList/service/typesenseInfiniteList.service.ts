...
export const getPaginatedList = async <T>({
    ...
    let searchParameters: SearchParams = {
        ...
        sort_by: `${orderValue.toString()}:asc,_text_match:asc`,
        page: paginationParams.page,
        filter_by: getFilters(paginationParams.filterBy),
    };

    const res: SearchResponse<any> = await typesenseClient
        .collections(col)
        .documents()
        .search(searchParameters);

    logger.debug(
        `Response of ${paginationParams.searchTextParam}:`,
        res.hits?.map(doc => doc.document.name),
    );
...
export const typesenseInfiniteListService = {
    getPaginatedList,
};
