...
export const useQuickStartAnimations = () => {
    ...
    //Animations
    const buttonWidth = useSharedValue<number | string>(defaultValues.buttonWidth)
    ...
    const myAnimatedContainer = useAnimatedStyle(() => {
        const getRadius = () => {
            if (isMenuOpen.value) {
                return withTiming(buttonRadius.value, {duration: 200, easing: Easing.in(Easing.ease)})
            }
            return withTiming(buttonRadius.value, {duration: 200, easing: Easing.in(Easing.ease)})
        }
        ...
        const getHeight = () => {
            if (isMenuOpen.value) {
                return withDelay(200, withTiming(buttonHeight.value, {duration: 200, easing: Easing.inOut(Easing.ease)}))
            }
            return withTiming(buttonHeight.value, {duration:200, easing: Easing.inOut(Easing.ease)})
        }

        const getWidth = () => {
            if (isMenuOpen.value) {
                return withTiming(buttonWidth.value, {duration: 200, easing: Easing.inOut(Easing.ease)})
            }
            return withDelay(200, withTiming(buttonWidth.value, {duration: 200, easing: Easing.inOut(Easing.ease)}))
    ...
    const quickStartContainerStyle = useAnimatedStyle(() => {
        const getOpacity = () => {
            if (isMenuOpen.value) {
                return withDelay(300, withTiming(isMenuOpen.value, {
                    duration: 100,
                    easing: Easing.inOut(Easing.ease)
                }))
            }
            return withTiming(isMenuOpen.value, {duration: 100, easing: Easing.inOut(Easing.ease)})
        }
    ...
    const iconStyle = useAnimatedStyle(() => {
        const getOpacity = () => {
            ...
            return withDelay(450, withTiming(isMenuOpen.value === 0 ? 1 : 0, {
                duration: 100,
                easing: Easing.inOut(Easing.ease)
            }))
        }
